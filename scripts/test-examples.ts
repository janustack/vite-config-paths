...
async function runExample(example) {
  ...
  try {
    // Install and build
    await execAsync('bun run install', { cwd: examplePath });
    await execAsync('bun run build', { cwd: examplePath });

    // Start preview server with timeout
    const server = spawn('bun', ['run', 'preview'], { cwd: examplePath });
    ...
    const testPromise = execAsync(`bun exec cypress run --config-file cypress.config.ts --env example=${example}`);
